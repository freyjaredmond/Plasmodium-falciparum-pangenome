...
paralog=pd.read_csv("PSEUDO/16_paralog_counts.txt", sep="\t")
...
for cluster in matrix.columns:
    ...
    if cluster in paralog_dict: 
        ...
        if occupancy in (1,2) and paralog_dict[cluster]==0:
            cloud_genes_no_ortholog.append(cluster)
        if occupancy== 16 and paralog_dict[cluster]==0:
            core_genes_no_ortholog.append(cluster)
        occupancy_to_ortho.append({
            "cluster":cluster,
            "occupancy":occupancy,
            "group":group,
            "paralog count":paralog_dict[cluster]})    
...
combined_filtered=df.merge(product, left_on="cluster", right_on="Gene ID", how="left")
combined_filtered = combined_filtered[~combined_filtered["Product Description"].str.contains("rif|rifin|stevor|var|pfemp1|erythrocyte membrane protein 1, PfEMP1|VAR-like", case=False, na=False)]
...
fig.savefig("PSEUDO/GRAPHS/paralog_box_grouped_filtered_PSEUDO.png", dpi=300)


#define groups and data to be compared

core_data=combined_filtered[combined_filtered["group"]=="Core"]["paralog count"]
soft_core_data= combined_filtered[combined_filtered["group"]=="Soft-core"]["paralog count"]
shell_data=combined_filtered[combined_filtered["group"]=="Shell"]["paralog count"]
cloud_data=combined_filtered[combined_filtered["group"]=="Cloud"]["paralog count"]
...
medians=combined_filtered.groupby("group")["paralog count"].median().sort_values()
...
sorted_df = combined_filtered.sort_values("paralog count", ascending=False)
